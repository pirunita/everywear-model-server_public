...
def data_loader(FLAGS, user_image_name, product_image_name):
    if FLAGS.is_upper is 0:
        user_image_pkl_dir = FLAGS.input_dir + '/body_pickle/'
    elif FLAGS.is_upper is 1:
        user_image_pkl_dir = FLAGS.input_dir + '/upper_pickle/'
    product_image_pkl_dir = FLAGS.product_dir + '/pkl/'
    
    with open(user_image_pkl_dir + user_image_name + '.pkl', 'rb') as f:
        try:
            image_data = pkl.load(f, encoding='latin-1')
        except EOFError:
            pass
    
    with open(product_image_pkl_dir + product_image_name + '.pkl', 'rb') as f:
        try:
            product_image_data = pkl.load(f, encoding='latin-1')
        except EOFError:
            pass
...
def data_loader(FLAGS, user_image_name, product_image_name):
    ...
    product_mask_dir = FLAGS.product_dir + 'cloth-mask/'
